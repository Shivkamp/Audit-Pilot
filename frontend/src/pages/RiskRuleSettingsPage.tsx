...
function RuleRow({ rule, canConfig, workspaceId, onDirtyChange }: RuleRowProps) {
  ...
  // Stale state fix: sync from rule prop after refetch/server-normalize
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // onDirtyChange is stable (useCallback in parent)
  ...
  return (
    <div className={`bg-white border rounded-xl p-5 shadow-card transition-all duration-150 ${isDirty ? 'border-blue-300 ring-1 ring-blue-200' : 'border-slate-200'}`}>
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {isDirty && (
              <span className="text-xs text-blue-600 font-medium bg-blue-50 px-1.5 py-0.5 rounded">unsaved</span>
            )}
          </div>
        </div>
      ...
      {canConfig && isDirty && (
        <button
          ...
          className="mb-3 px-3 py-1.5 text-xs font-semibold text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition-colors cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-500 disabled:opacity-60 disabled:cursor-not-allowed"
        >
...
export function RiskRuleSettingsPage() {
  ...
  // Clear dirty state when rules refetch (e.g. after reset-defaults)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <PageHeader
        ...
        subtitle="Customize detection thresholds and rule settings for this workspace"
        actions={
          canConfig ? (
            <div className="flex items-center gap-2 flex-wrap">
